...
# @@File             :  entrypoint.sh
# @@Description      :  Entrypoint file for mailman
...
SERVICE_USER="${SERVICE_USER:-$mailman}"  # execute command as another user
SERVICE_GROUP="${SERVICE_GROUP:-mailman}" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Secondary ports
...
if [ "$ENTRYPOINT_FIRST_RUN" != "no" ]; then
  ...
  if [ "$CONFIG_DIR_INITIALIZED" = "no" ] || [ "$DATA_DIR_INITIALIZED" = "no" ]; then
    [ "$ENTRYPOINT_MESSAGE" = "yes" ] && echo "Executing entrypoint script for mailman"
  fi
  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
